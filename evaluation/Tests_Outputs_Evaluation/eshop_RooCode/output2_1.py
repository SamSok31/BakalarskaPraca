...
class Product:
    ...
    def update_stock(self, quantity: int) -> bool:
        ...
        if self.stock + quantity < 0:
            logger.warning(f"Cannot update stock for product {self.product_id}: insufficient stock")
            return False
        self.stock += quantity
        logger.info(f"Product {self.product_id} stock updated: {self.stock - quantity} -> {self.stock}")
        return True
...
class EShop:
    ...
    def delete_user(self, user_id: str) -> bool:
        ...
        for reservation in cart.reservations.values():
            ...
            # If product no longer exists, just remove the reservation
    ...
    def add_product(self, product_id: str, name: str, price: float, stock: int) -> bool:
        ...
        if product_id in self.products:
            ...
            product.price = price
            product.update_stock(stock - product.stock)  # Adjust stock difference
            logger.info(f"Product {product_id} updated: price={price}, stock={stock}")
        else:
            # Add new product
            self.products[product_id] = Product(product_id, name, price, stock)
            logger.info(f"Product {product_id} added: name={name}, price={price}, stock={stock}")
        return True
    ...
    def checkout(self, user_id: str) -> Optional[str]:
        ...
        # Check if all reserved products still exist in stock
        for reservation in cart.reservations.values():
    ...
    def _handle_order_cancellation(self, order: Order) -> None:
        ...
        for item in order.items:
            if item.product_id in self.products:
                # Product exists, return stock
                self.products[item.product_id].update_stock(item.quantity)
            else:
                # Product doesn't exist, recreate it with the stored price
