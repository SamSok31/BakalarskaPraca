...
class EShopSystem:
    ...
    def checkout(self, user_id: str) -> Optional[str]:
        ...
        with self._lock:
            ...
            # Validate all reserved products still exist and calculate total using current prices
            for product_name, cart_item in cart.items.items():
                if product_name not in self.products:
                    # Product no longer exists - checkout fails
                    return None
                
                # Use current price for checkout
                current_price = self.products[product_name].price
                order_items.append(CartItem(product_name, cart_item.quantity, current_price))
                total_price += cart_item.quantity * current_price
