# Let's update the EShopBackend class to address the issues:
# 1. Prevent negative stock when adding products.
# 2. Prevent returning products to stock when cancelling an already cancelled order.
...
class EShopBackend:
    ...
    def add_product(self, name: str, price: float, stock: int) -> None:
        if stock < 0:
            return  # Ignore if stock is negative
